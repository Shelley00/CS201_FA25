...
```{r, message=FALSE, warning=FALSE}
penguins %>%
  relocate("Individual ID", .after = "Sample Number")
...
```{r, message=FALSE, warning=FALSE}
penguins %>%
  mutate("flipper_length_km" = "flipper_length_mm")
...
penguins %>%
  ...
  summarize("Culmen Depth mm" = mean("Culmen Depth mm, na.rm TRUE"),
...
1.  est whole number.Create a new object called `final_boss` that satisfies all of the following:
    ...
    5.  Group by `species`, `island`, and `flipper_rank`, and summarize the *average body mass* (name this column `avg_mass_g`), rounded to the near
